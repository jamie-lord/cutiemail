...
cutiemail is one Node process over synchronous SQLite, chosen for simplicity and
...
project targets. An 8-core laptop runs the same rigs 5-10× faster.
...
each unit of work is small and bounded**, which reduces the performance design to one rule:
...
- `raw(uid)`: one message body, one row, fetched only when a command needs bytes
...
195 MB of heap**. Because SQLite is synchronous, those two seconds froze everyone: with
three concurrent readers, a brand-new connection waited 4.6 s for its greeting and an
...
- **Write-backlog budget (256 MiB).** A client that requests a large message and then stops
  reading leaves the response buffered in the process. One big fetch per connection is
...
These are the measured limits of the single-threaded synchronous design, all of them far beyond
the intended scale of one domain and a handful of people:
...
concurrent load. That is a disproportionate risk for the mission, recorded here as the next move
if that complaint ever materialises.
...
enter IDLE and then drop abruptly, the classic leak trigger. It samples live memory
