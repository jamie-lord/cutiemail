...
| 5 | Message format: RFC 5322 + MIME | RFC 5322, 2045-2049, 2047, 6532 | **[building]** reference parsers with switchable defects (`src/message/`) + **[adopt]** torture corpora to come | Foundational — every surface touches it. **Landed:** RFC 5322 §2.1/§2.2/§3.3/§3.4.1/§3.6 (structure, header-injection defence, date-time, addr-spec, field validation) and RFC 2045 §4/§5/§6 (MIME-Version, Content-Type, Content-Transfer-Encoding — the MIME-confusion surface), each negative-controlled. Still to build: multipart trees/boundaries (§2046), RFC 2047 encoded-words, EAI/UTF-8 (6532), the torture corpora. *Lever: parse modern strictly; reject rather than heroically repair, each rejection recorded.* |
