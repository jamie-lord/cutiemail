...
|---|---|---|---|---|
| 2 | Submission + AUTH | RFC 6409, 4954, SCRAM 5802 | **[building]** SCRAM proof crypto landed (`src/auth/scram.ts`); message exchange + submission fix-up to come | AUTH mechanisms, STARTTLS-before-AUTH, submission fix-up (Date/Message-ID). **SCRAM core landed:** the password-never-sent proof algebra (PBKDF2 → ClientProof/ServerSignature, server-side verify) with real `node:crypto`, pinned to the RFC 5802 §5 worked-example vectors, parameterized for SHA-1 (vector) and SHA-256 (production). Still to build: SASLprep, the client-first/server-first/client-final/server-final message exchange + parsing, the AUTH command/state machine, STARTTLS-before-AUTH enforcement. *Lever: SCRAM-SHA-256 + PLAIN-over-TLS only; refuse plaintext AUTH.* |
...
| 7 | DKIM | RFC 6376 | **[building]** verify path complete end-to-end (`src/crypto/dkim-*.ts`, `src/register/crypto/`); Ed25519 + signing direction to come | Sign outbound, verify inbound. **DKIM verify path complete end-to-end:** the four simple/relaxed canon algorithms (pinned to the RFC 6376 §3.4.5 vectors), the §3.5 tag-list parser (duplicate/required/unknown tags), the §3.7 body hash (real SHA-256 over the canon body vs `bh=`), and the §3.7-step-2 header hash + **RSA signature verification**, and the §5 **signing (outbound) direction** — a signed message ROUND-TRIPS through the verifier (real keypair via `node:crypto`, tamper-detected, key-strength + algorithm gates) — all negative-controlled. Still to build: Ed25519 (RFC 8463), the `l=` body-length limit, `x=` expiry, and key-record DNS retrieval. |
