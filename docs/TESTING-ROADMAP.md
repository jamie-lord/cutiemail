...
| 3 | Outbound SMTP client / delivery | RFC 5321 (client half), 3464 (DSN) | **[build]** reverse-sink + DNS mock + queue tests | The sending leg. Makes the **141 client-binding requirements** already in the register testable. MX lookup, retry/backoff, bounce generation, crash recovery. |
| 4 | IMAP server | RFC 9051 (IMAP4rev2) | **[adopt]** Dovecot `imaptest` + **[build]** conformance | The read leg. *Lever (largest in the project): IMAP4rev2 + a curated few extensions (IDLE, MOVE, CONDSTORE, SPECIAL-USE); refuse the legacy long tail.* |
| 5 | Message format: RFC 5322 + MIME | RFC 5322, 2045-2049, 2047, 6532 | **[build]** parser conformance + **[adopt]** torture corpora | Foundational — every surface touches it. Header parsing, MIME trees, transfer-encodings, EAI/UTF-8, **header-injection defence**. *Lever: parse modern strictly; reject rather than heroically repair, each rejection recorded.* |
