...
so it sidesteps the human-password length policy (the NIST SP 800-63B 8-character floor) entirely
and is far stronger by construction. It is never taken on argv (which `ps` exposes) nor stored
anywhere recoverable. `list` shows names and dates, never secrets; `remove` revokes one, honoured live.
