...
- **Inbound MTA-STS publication** (RFC 8461): without a published policy, an active
  attacker can downgrade mail sent *to* this server to plaintext
...
HTTP" is a scope boundary, and to cross it for two static documents is
not justified when:
...
1. **The server-derived part of MTA-STS is the policy *content*, not its
   ...
   `id` derived as a content hash (it changes exactly when the policy changes,
   and identical re-runs stay diffable). Five static lines are what static
   hosts are for: the file can live on any HTTPS host that serves
   `https://mta-sts.<domain>/.well-known/mta-sts.txt`. The generated policy
   round-trips through our *own* RFC 8461 parser (the code that
   enforces such policies outbound), proven with a negative control (the parser
   refuses an unlisted MX).
2. **The certificate cost is real and lands on our weakest flank.** A built-in
   ...
   `autoconfig.<domain>`), and expands exactly the part of the ops story (cert
...
- If built-in ACME is ever justified (it needs an HTTP listener anyway),
  MTA-STS publication and autoconfig follow at near-zero marginal cost.
