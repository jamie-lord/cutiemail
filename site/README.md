...
The docs and marketing site for cutiemail, served at [cuti.email](https://cuti.email).

It is a small, hand-rolled static generator, no framework, in keeping with the project. The
homepage (`pages/home.html`) is bespoke, built to the brand design. The generator renders every
docs page from the repository's **own markdown** (`README.md`, `docs/*`, the ADRs), so the published
docs and the repo docs are the same files. There is nothing to keep in sync.
...
contributing and security policies, or this directory. There is no manual step, and a new ADR
appears automatically (the build generates the sidebar from `docs/decisions/`).

## What lives where

- `build.mjs`: the generator. Markdown → HTML, internal-link rewriting, sidebar, mermaid.
- `styles/site.css`: the brand design system (tokens and components).
- `pages/home.html`: the homepage content.
- `assets/`: Cuti (the mascot) as SVGs.
- `wrangler.jsonc`: the Cloudflare Workers static-assets and custom-domain config.
