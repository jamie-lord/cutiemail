...
test('DMARC passes when an aligned DKIM or SPF identifier passed', async () => {
  ...
  const viaDkim = await checkDmarc({ rawMessage: msg('example.com'), dkimResult: 'pass', dkimDomain: 'example.com', spfResult: 'none', spfDomain: '', resolveTxt: rec });
  assert.equal(viaDkim.verdict, 'pass');
  assert.equal(viaDkim.policy, 'reject', 'the published policy is reported');

  const viaSpf = await checkDmarc({ rawMessage: msg('example.com'), dkimResult: 'none', dkimDomain: null, spfResult: 'pass', spfDomain: 'example.com', resolveTxt: rec });
...
test('a passing but UNALIGNED identifier does not satisfy DMARC', async () => {
  ...
  const out = await checkDmarc({ rawMessage: msg('example.com'), dkimResult: 'pass', dkimDomain: 'unrelated.test', spfResult: 'fail', spfDomain: 'unrelated.test', resolveTxt: rec });
...
test('relaxed alignment matches a subdomain via the organizational-domain fallback', async () => {
  ...
  const out = await checkDmarc({ rawMessage: msg('mail.example.com'), dkimResult: 'pass', dkimDomain: 'example.com', spfResult: 'none', spfDomain: '', resolveTxt: rec });
...
test('strict alignment requires an exact domain match', async () => {
  ...
  const out = await checkDmarc({ rawMessage: msg('mail.example.com'), dkimResult: 'pass', dkimDomain: 'example.com', spfResult: 'none', spfDomain: '', resolveTxt: dmarcAt({ '_dmarc.mail.example.com': 'v=DMARC1; p=reject; adkim=s' }) });
...
test('no DMARC record is "none"; a DNS error is "temperror"', async () => {
  assert.equal((await checkDmarc({ rawMessage: msg('nodmarc.test'), dkimResult: 'pass', dkimDomain: 'nodmarc.test', spfResult: 'none', spfDomain: '', resolveTxt: async () => [] })).verdict, 'none');
  assert.equal(
    (
      await checkDmarc({
