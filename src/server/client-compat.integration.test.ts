...
test('the Thunderbird account-setup sequence completes against the server', async () => {
  ...
  try {
    assert.match(await s.greeting(), /\* OK \[CAPABILITY IMAP4rev2 IDLE UIDPLUS SPECIAL-USE CONDSTORE AUTH=PLAIN\]/);

    const cap = await s.run('t1', 'CAPABILITY');
    assert.match(cap, /^\* CAPABILITY IMAP4rev2 IDLE UIDPLUS SPECIAL-USE CONDSTORE AUTH=PLAIN\r$/m, 'CAPABILITY answers as a command');
