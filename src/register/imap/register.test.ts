/**
 * The IMAP register's integrity gates — the same discipline the SMTP, message and
 * crypto registers hold, via the shared machinery in ../gate.ts. The load-bearing
 * one is `verbatim`: every `text` must be a genuine substring of RFC 9051.
 */

import { test } from 'node:test';
import assert from 'node:assert/strict';
import { IMAP_REQUIREMENTS, EXTRACTED_SECTIONS } from './index.ts';
import type { RequirementDef, Level } from '../types.ts';
import { collapse, loadSpec } from '../gate.ts';

const reqs = IMAP_REQUIREMENTS as readonly RequirementDef[];
...
test('requirement ids agree with the section and RFC they cite', () => {
  for (const r of reqs) {
    const rfcNum = (r.rfc ?? 'rfc9051').replace('rfc', '');
    const expected = `R-${rfcNum}-${r.section}-`;
    assert.ok(r.id.startsWith(expected), `${r.id} claims §${r.section} of ${r.rfc ?? 'rfc9051'}; id should start "${expected}"`);
  }
});
