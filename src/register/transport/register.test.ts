/**
 * The transport register's integrity gates — the same discipline the other
 * registers hold, via the shared machinery in ../gate.ts.
 */

import { test } from 'node:test';
import assert from 'node:assert/strict';
import { TRANSPORT_REQUIREMENTS, EXTRACTED_SECTIONS } from './index.ts';
import type { RequirementDef, Level } from '../types.ts';
import { collapse, loadSpec } from '../gate.ts';

const reqs = TRANSPORT_REQUIREMENTS as readonly RequirementDef[];
...
test('requirement ids agree with the section and RFC they cite', () => {
  for (const r of reqs) {
    const rfcNum = (r.rfc ?? 'rfc8461').replace('rfc', '');
    const expected = `R-${rfcNum}-${r.section}-`;
    assert.ok(r.id.startsWith(expected), `${r.id} claims §${r.section} of ${r.rfc ?? 'rfc8461'}; id should start "${expected}"`);
  }
});
