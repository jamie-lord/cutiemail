...
`src/register/EXTRACTING.md`. Read it fully before you write tests.
...
2. **Assert the class the spec constrains, never a tighter thing.** RFC 5321 mostly fixes
   ...
   doubt, assert `severity(reply) === 5`, not `reply.code === 550`. The register notes mark
   these landmines per requirement. Read them.
...
4. **A MAY has no failure state.** If a requirement is MAY, you may only return `satisfied`,
   `observed` (with the branch that was taken), or `inconclusive`. A `violated` return for a
   MAY throws, by design. If you want to fail a server for a MAY, you have misread the
   requirement.

5. **Every wire-testable requirement needs a Mutant.** A test never demonstrated to fail against a
   broken server is faith, not evidence. Pair each `TestCase` for a `wire` requirement with a
   `Mutant` that names the mutant-server defect that must trip it. The coverage report marks a
   test without a proven mutant as `test-only`, not `covered`.
...
hosts (isi.edu and others). A conformance run against one is someone else's incident.
...
without a declared need, lies on a server configured differently.

If a test needs an EHLO extension, declare it in `needs.ehlo`. A server that does not
advertise it is out of scope for that test (`inconclusive`), not non-conformant.

## Judging: observe, then conclude

A test body may only observe (through `Conn`) and return a `Judgement`. It cannot see the
register, decide its own `Outcome`, or reach other tests. Keep bodies small: drive the
exchange, read the replies, and return one of these:
...
- `{ kind: 'inconclusive', reason }`: could not tell (rare. The runner usually handles this).

Use `conn.expectQuiet(ms)` for requirements whose violation is *action where there should be
none*: a reply to an unterminated command line, or a honoured bare LF. Silence is the pass.

## Evidence

You get evidence for free: the runner captures the full byte transcript and the last reply.
Make `intent` and `rationale` useful. They are what a human reads during triage. Calibration
(task #23) assumes the suite is wrong until the transcript proves otherwise.
...
coverage report reads those, so an unregistered module is invisible. The report shows this as
uncovered requirements, not as a pass.
...
- **SHOULD / MAY**: a `TestCase` (whose `run` returns `violated` on the decline, and the outcome
  model maps that to `permitted-latitude`, never a finding) + a `LatitudeControl` in `CONTROLS`
  that gives the `follows`/`declines` mutant defects, proven by `verifyLatitudeControls`. NEVER
  give a SHOULD/MAY a negative control. A declined SHOULD is not a violation, so a mutant cannot
  "catch" it, and `coverage.ts` blocks any credit that way.
- **Delivery-path (invisible on the connection)**: dot-un-stuffing, case preservation, trace
  ...
  defect corrupts the relayed message, proven by `verifySinkControls`. This is inconclusive without
  a sink (a plain run against a server we cannot make relay to us), never a false finding.

## alsoProves: one defect, several requirements

Sometimes two register entries state the SAME wire behaviour in different sections (for example, an
unrecognised NOOP violates both §4.5.1-b and §4.3.2-e). In that case, add an `alsoProves` array to
the mutant. Each entry is a `{ requirement, why }` for a requirement that the caught test's
exchange also demonstrates. This is a DELIBERATE, per-claim credit, NOT automatic. An invariant
test bounds each `alsoProves` to the caught test's own `requirement`/`alsoTouches`, and
`coverage.ts` refuses to credit a non-MUST this way (SHOULD/MAY go through latitude). Never use it
to hide a gap.
...
- You ran the module against the mutant server, and you saw each test go red on its defect and
  green on the clean baseline. A test that only passes is half a test.
