...
test('verifyPassword: right password passes, wrong password and unknown login fail', async () => {
  ...
  assert.equal(await reg.verifyPassword('alice', 'correct horse'), true);
  assert.equal(await reg.verifyPassword('alice', 'wrong'), false, 'wrong password rejected');
  assert.equal(await reg.verifyPassword('bob', 'correct horse'), false, 'unknown login rejected');
});
...
test('a disabled account fails auth even with the right password, until re-enabled', async () => {
  ...
  assert.equal(await reg.verifyPassword('alice', 'pw'), false, 'disabled → no auth');
  assert.equal(reg.lookup('alice')?.enabled, false);
  reg.setEnabled('alice', true);
  assert.equal(await reg.verifyPassword('alice', 'pw'), true, 're-enabled → auth');
});
...
test('a login is case-insensitive identity across every login-keyed operation', async () => {
  ...
  for (const spelling of ['alice', 'Alice', 'ALICE', 'aLiCe']) {
    assert.equal(await reg.verifyPassword(spelling, 'correct horse'), true, `auth as ${spelling}`);
  ...
  assert.equal(await reg.verifyPassword('ALICE', 'wrong'), false, 'wrong password still rejected');
  assert.equal(await reg.verifyPassword('nobody', 'correct horse'), false, 'unknown login still rejected');

  reg.setEnabled('ALICE', false);
  assert.equal(reg.lookup('alice')?.enabled, false, 'disable reaches the row whatever case is typed');
  assert.equal(await reg.verifyPassword('alice', 'correct horse'), false, 'disabled → no auth');
  reg.setEnabled('alice', true);
  ...
  assert.equal(await reg.verifyPassword('alice', secret), true, 'app password added as ALICE authenticates alice');
  assert.deepEqual(
...
test('a password rotation typed in the wrong case replaces the credential, never forks the account', async () => {
  ...
  assert.equal(await reg.verifyPassword('alice', 'new password'), true, 'the rotation took effect');
  assert.equal(await reg.verifyPassword('alice', 'old password'), false, 'the old credential no longer authenticates');
});
...
test('credentials and routing survive a close/reopen of the same database file', async () => {
  ...
  try {
    ...
    assert.equal(await reg2.verifyPassword('alice', 's3cret'), true, 'password verifies after reopen');
    assert.equal(await reg2.verifyPassword('alice', 'nope'), false);
...
test('app passwords: each authenticates like the primary, is independently revocable (ADR 0017)', async () => {
  ...
  assert.equal(await reg.verifyPassword('alice', 'primary-password'), true, 'the primary still works');
  assert.equal(await reg.verifyPassword('alice', phone), true, 'the phone app password authenticates');
  assert.equal(await reg.verifyPassword('alice', laptop), true, 'the laptop app password authenticates');
  // A wrong secret and another account are still refused.
  assert.equal(await reg.verifyPassword('alice', 'not-a-real-secret'), false);
  assert.equal(await reg.verifyPassword('bob', phone), false, 'the app password is scoped to its owner');

  // Revoke ONE: it stops working; the others (and the primary) are untouched.
  assert.equal(reg.removeAppPassword('alice', 'phone'), true);
  assert.equal(await reg.verifyPassword('alice', phone), false, 'the revoked app password no longer authenticates');
  assert.equal(await reg.verifyPassword('alice', laptop), true, 'a sibling app password still works');
  assert.equal(await reg.verifyPassword('alice', 'primary-password'), true, 'the primary is unaffected');
...
test('app passwords: a disabled account fails auth on the app password too', async () => {
  ...
  assert.equal(await reg.verifyPassword('alice', secret), true);
  reg.setEnabled('alice', false);
  assert.equal(await reg.verifyPassword('alice', secret), false, 'disabling the account disables its app passwords');
  reg.setEnabled('alice', true);
  assert.equal(await reg.verifyPassword('alice', secret), true, 're-enabling restores them');
});
...
test('negative control: the database never contains the plaintext password', async () => {
  ...
  try {
    ...
    assert.equal(await r2.verifyPassword('a', 'password-two'), false, "a's key does not match b's password");
    assert.equal(await r2.verifyPassword('b', 'password-one'), false);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('rotating a password does not move the postmaster mailbox to another account', async () => {
  ...
  assert.equal(await reg.verifyPassword('alice', 'password-three'), true, 'the new password works');
  assert.equal(await reg.verifyPassword('alice', 'password-one'), false, 'the old one does not');
});
