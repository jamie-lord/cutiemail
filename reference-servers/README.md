...
daemon with its own config, own spool, a high port, no root, and the host's system mail untouched:
...
accepts NUL/BEL control octets) or our own minimal test config. None was a suite bug or an RFC
