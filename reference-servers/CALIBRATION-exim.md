...
Exim is one of the two most spec-scrutinised MTAs alive, and one of the four that the conformance
suite is calibrated against. This run uses a **natively-installed Exim 4.99.4** (`brew install
exim`). It runs Exim as an **isolated, unprivileged test daemon** with its own config, its own
spool, a high port, and no root. It does **not mutate the host's system mail**.
...
**ZERO false positives.** Every finding traced to a real cause. None was a suite bug or a
misreading of the RFC. This is the first time that the runner, the reply reader, and the four-state
grading were validated end-to-end against a production MTA.

## Triage of every finding

| # | Requirement | Reported | Triage | Resolution |
|---|---|---|---|---|
| 1 | R-5321-2.3.5-g (MUST) | `RCPT TO:<postmaster>` (bare) drew `550 relay not permitted` | **Our reference config, NOT a suite bug.** The suite's expectation is correct (§4.5.1: bare postmaster MUST be accepted). My minimal ACL qualified the domain-less `postmaster` to `postmaster@exim.example.com`, which was not in `local_domains`. | Added `accept local_parts = postmaster` to the ACL; the finding **cleared** on re-run (58 → 59 OK). Proves the finding tracked the server's real config, and the check is sound. |
...
The 7 inconclusive results are all honest: two size floors
(`longLocalPartRecipient`/`longDomainRecipient` not declared for this config), EXPN refused with 550
(Exim's anti-harvesting posture, so no expansion is observable), and four sink cases (no
relay-to-sink configured for this run).
...
four-state grading makes **no false accusation** against Exim across 59 conformant behaviours. The
two findings it does raise are a real, byte-verified divergence, not an artefact. Alongside
Postfix, mox, and aiosmtpd, the instrument is validated against four independent implementations.
...
A re-run of this calibration against a freshly-launched native Exim 4.99.4, after substantial
server and storage work, gives an **identical result**: the same two genuine bare-LF divergences
(R-5321-2.3.8-a, R-5321-4.1.1.4-i), and the same zero false positives across 59 conformant Exim
behaviours. The corpus and the runner are separate from the server code, so changes to the live
servers and storage do not move the calibration. This confirms that the separation holds.
