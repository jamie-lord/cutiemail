...
an **isolated, unprivileged test daemon** — its own config, its own spool, a high port, no root,
...
**The number that matters: ZERO false positives.** Every finding was triaged to a real cause;
...
not declared for this config), EXPN refused with 550 (Exim's anti-harvesting posture — cannot
observe an expansion), and four sink cases (no relay-to-sink configured for this run).
